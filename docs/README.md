...
**Outside `docs/`:** `../guardrails.config.json` (architecture as machine-checked config) · `../scripts/guardrails.mjs` (the ten checks that block deploy) · `../src/styles/tokens.css` and `../src/styles/base.css` (the style layers) · `../src/lib/i18n.ts` (locales, and every URL) · `../src/lib/strings.ts` (interface copy per language) · `../src/lib/vocabulary.ts` (domain words) · `../design/` (mockups).
