...
| Publishing | **Locally or from GitHub Actions — never Cloudflare's own builder.** `publish.yml` runs the same `deploy.mjs` nightly at 23:07 UTC — just after midnight in Zurich — and again at 03:07, because GitHub starts scheduled jobs up to five hours late | Building on Cloudflare cost five hours across six failed deploys, none about the site: a password copied into a second place, a "retry" that replays the old commit, a Git link that dropped, an IPv6-only database host. Building elsewhere removes all of it |
...
| "Today", "this weekend" | Baked in at the nightly build; whether a market is open *now* is filled in by the browser | A page written at night cannot know 14:00. Without JavaScript the page still says "Heute" and the hours |
...
| `GITHUB_DISPATCH_TOKEN` | Cloudflare Pages secret | Fine-grained token, Actions: write, this repo only. A button press runs `publish.yml` now instead of at the nightly run (`functions/_publish.ts`) |
...
**Collection.** `functions/_middleware.ts` counts page views at the edge before the HTML is served — no client JS, so a blocker cannot remove them. It reads the page's own `<meta name="fynda:market|city|region|next">` tags off the response, so a view carries the market id, town, canton and days to the next date; it records 404s as `not_found`; and it writes every robot to `crawler_hits` (name, role — training / search / *user*, meaning a person asked an AI about that page — network, path) instead of dropping it. `functions/e.ts` takes the browser's events: clicks, filters, saves, and since 2026-09-17 `page_leave` (visible time, scroll depth, sections seen), `web_vitals`, `js_error`, rage and dead clicks, newsletter form start, language switches. Both write `analytics_events` through `functions/_collect.ts`, which adds what Cloudflare knows free on every request — town, canton, timezone, network, connection RTT, protocol, EU flag — plus the browser's first language and the previous page on our site. `props` shapes are fixed by a check constraint per event: obey it exactly, or a second vocabulary splits the numbers. **Every row carries `page_type`** — the one property that would have shown v1's collapse.
...
**Retention.** Nothing is deleted; there is deliberately no prune, and no aggregate table — at ~300 rows a day Metabase reads the raw table directly. If that ever gets slow, an aggregate arrives as its own migration, on evidence. The daily hash key is `ANALYTICS_SALT + date`; the salt is permanent, so rows stay linkable to an IP for whoever holds the salt. That is accepted: keep all the data, no re-hashing job (Delfim, 2026-09-19). The privacy page promises hashed, unreadable IPs and nothing more, which is what the code does.

**Reading.** Metabase under Docker on this machine, six dashboards (72 questions) built by `metabase/dashboards.py`, `metabase/README.md`. Views for it: `analytics_events_classified` (raw + bot verdict), `analytics_visitor_days`, `analytics_sessions` (cookieless sittings, bounce), `analytics_engagement` (page_leave flattened), `crawler_hits`. It reads as `metabase_ro`, which cannot see `reports` or `market_private` but can read the two triage views. Search Console is imported from the free CSV export by `scripts/import-gsc.mjs` — Google keeps 16 months and never backfills, so the habit matters more than the schedule.
