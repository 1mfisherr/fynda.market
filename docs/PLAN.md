...
- **The home page shows the goods, then asks one question.** The three-field search card is gone: it stood between a visitor and the markets, which is the shape of the one competitor whose home page shows no listings at all. In its place — the claim as a number (*"157 Flohmärkte, jeder Termin mit Quelle und Prüfdatum"*), **six markets from this weekend** (two at full size, four compact, one per town, biggest towns first), then **"Wo sind Sie?"**. Picking a town is remembered in the browser, no account, and the next visit opens with a one-tap shortcut into that town's page — the best-earning page type on the site. `docs/PAGES.md` §Home.
