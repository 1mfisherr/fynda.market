...
- **Analytics.** Page views counted at the edge, interactions from the browser, both into our Postgres since 2026-09-06; audited and corrected 2026-09-10. Honest baseline: **8–20 Swiss visitors a day, 7–16 from Google**; two thirds of raw events are crawlers that pass the user-agent filter. Search Console week one: 1,094 impressions, 12 clicks, market pages ahead of city pages.
...
1. **Read the numbers before judging the product.** One more Search Console export and two weeks of clean events. First call to make from data rather than taste: whether the home page's search card stays (`filter_changed` answers it).
2. **A subscribable calendar feed per town.** Per-market `.ics` exists; a town feed needs no address, no consent, no deliverability, and nobody in the category has one.
3. **"Less often" / "pause" on the unsubscribe page.** Token, page and row all exist; today the page is a dead end.
4. **Resend webhook for opens and clicks.** Nothing else can retire dead addresses, and dead addresses drag sender reputation down.
5. **The one-week-out confirmation mail to organisers.** "Yes, it's on" — two buttons, no login. 23 markets qualify, ~35 dates over 120 days. `organiser_claims` can hold the answer; who is asked, how often, and what silence means are undecided.
6. **Ask organisers for size and indoor/outdoor.** The two facts that decide a forty-minute drive, held for none of 161 markets, and unobtainable by scraping. This is what the organiser page is for.
7. **Country page** `/{locale}/{country}/`. Route allowed, nothing built; breadcrumbs join it when it exists.
8. **German text search.** Settle the compound-word question (`STACK.md`) before any search box exists. Not urgent: v1 saw 64 searches in 26 days against 281 filter uses.
