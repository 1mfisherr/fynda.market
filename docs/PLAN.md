...
1. **Organisers, round one.** 22 organisers have an address and a personal link (loaded 2026-09-19 from the export; nothing sent). All 22 run occasional or annual markets, which is the case the seven-day mail is for (Delfim, 2026-09-19). The welcome mail for people who did not ask is written and approved (2026-09-20): what fynda.market is, their link, a way out, signed Delfim; no mention of the predecessor site. **To send:** GitHub → Actions → *Organiser welcome* → first with *test* = your address to read it in your inbox, then with *send* ticked. One row per welcome in `organiser_mail_sends` (migration applied 2026-09-20), so it cannot mail anyone twice; the claim-approve path logs its welcome the same way. Then `ORGANISER_SENDING=on` for the daily mail. Afterwards, e-mails for the ~90 with only a website: `scripts/import-organiser-emails.mjs <csv> --apply` loads them, the same workflow welcomes them. **Read the answer rate after four weeks** (`organiser_funnel`): >25% extend, 10–25% keep as a data feed, <10% stop building for organisers.
