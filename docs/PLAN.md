...
0. **Organiser page — done 2026-09-21.** Built from `PAGES.md` §Organiser page and driven live with the Carouge link at 375 and 1440: the opening, one-tap confirm (proven: "Confirmé par vous le 21 septembre" came back on the page, then reverted), the dates with times and Cancel, the facts, tags, "in your words", a save that returns to the page. Migration applied live the same day. The people count shows only from 20 up — today the busiest market has 15 a month, so nobody sees a number yet. Left: a native speaker's pass over DE/FR/IT.
1. **Organisers, round one.** 63 organisers have an address and a personal link (22 loaded 2026-09-19 from the export, 41 more with the 64 markets on 2026-09-20; nothing sent — the welcome goes to all 63 at once, the script takes everyone with a link and no welcome row; Delfim decided 2026-09-20 to send to all 63, after step 0). The test preview arrived 2026-09-20 and read well. All 22 run occasional or annual markets, which is the case the seven-day mail is for (Delfim, 2026-09-19). The welcome mail was rewritten 2026-09-21 as a letter from Delfim (who he is, why he's writing, what the organiser gains, the link, a way out, thanks) after three shorter drafts were rejected — "one tap, no account" lines read as a machine. A plain hello for everyone: only four of the 63 names are a person's. Dry run read well; **not yet sent — Delfim sends it himself.** **To send:** GitHub → Actions → *Organiser welcome* → *Run workflow* → your address in the **third** box ("Send a preview…"), everything else empty → read it in your inbox; then again with *send* ticked. Three attempts on 2026-09-20 sent nothing: two ran *Organiser mail* (the seven-day one, no test box) and one left the box empty — every Resend send shows in `newsletter_events`, so "did it leave?" is a query, not a guess. One row per welcome in `organiser_mail_sends` (migration applied 2026-09-20), so it cannot mail anyone twice; the claim-approve path logs its welcome the same way. Then `ORGANISER_SENDING=on` for the daily mail. Afterwards, e-mails for the ~90 with only a website: `scripts/import-organiser-emails.mjs <csv> --apply` loads them, the same workflow welcomes them. **Read the answer rate after four weeks** (`organiser_funnel`): >25% extend, 10–25% keep as a data feed, <10% stop building for organisers.
